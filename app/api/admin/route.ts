...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    products: products.map(product => ({ ...product, price: Number(product.price), inPersonPrice: Number(product.inPersonPrice), compareAtPrice: product.compareAtPrice ? Number(product.compareAtPrice) : null, cost: Number(product.cost) })),

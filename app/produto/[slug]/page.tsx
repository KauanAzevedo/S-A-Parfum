...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  const [related,buyers]=await Promise.all([prisma.product.findMany({where:{status:"ACTIVE",id:{not:product.id},OR:[{categoryId:product.categoryId},{gender:product.gender}]},include:{images:{orderBy:{position:"asc"},take:1}},orderBy:{createdAt:"desc"},take:4}),prisma.order.findMany({where:{customerId:{not:null},status:{in:["PAID","PREPARING","SHIPPED","DELIVERED"]},items:{some:{productId:product.id}}},select:{customerId:true}})]);
  const verifiedBuyers=new Set(buyers.map(order=>order.customerId));
  const distribution=[5,4,3,2,1].map(stars=>({stars,count:product.reviews.filter(review=>review.rating===stars).length,percent:product.reviews.length?Math.round(product.reviews.filter(review=>review.rating===stars).length/product.reviews.length*100):0}));
  return <><Header/><main className={`product-page product-luxury${product.stock<1?" product-sold-out":""}`}><div className="container-site">
    ...
    <section id="avaliacoes" className="product-reviews"><div className="product-reviews-heading"><p className="eyebrow">Opiniões de clientes</p><h2>Avaliações dos clientes</h2></div>{product.reviews.length>0&&<div className="review-overview"><div><strong>{rating.toFixed(1)}</strong><span>{"★".repeat(Math.round(rating))+"☆".repeat(5-Math.round(rating))}</span><small>Baseado em {product.reviews.length} {product.reviews.length===1?"avaliação":"avaliações"}</small></div><div className="review-distribution">{distribution.map(row=><div key={row.stars}><small>{row.stars} estrelas</small><span><i style={{width:`${row.percent}%`}}/></span><b>{row.percent}%</b></div>)}</div></div>} {!product.reviews.length&&<p className="product-review-empty">Seja a primeira pessoa a avaliar esta fragrância.</p>}{product.reviews.length>0&&<div className="product-review-list">{product.reviews.map(review=><article key={review.id}><div className="stars">{[1,2,3,4,5].map(value=><Star key={value} size={15} fill={value<=review.rating?"currentColor":"none"}/>)}</div><p>“{review.comment}”</p><footer><b>{review.user.name}</b><small>{new Intl.DateTimeFormat("pt-BR").format(review.createdAt)}{verifiedBuyers.has(review.userId)?" · ✓ Compra verificada":""}</small></footer></article>)}</div>}<ReviewForm target="product" productId={product.slug}/></section>

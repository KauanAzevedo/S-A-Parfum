...
export default async function Home() {
  ...
  return <><Header/><main>
  <section className="hero commercial-hero"><div className="container-site hero-content"><p className="eyebrow">Perfumaria original</p><h1>Sua presença começa<br/><em>pela fragrância.</em></h1><p className="hero-copy">Uma curadoria de perfumes importados e árabes <b>100% originais.</b></p><div className="hero-offer"><strong className="hero-pix">8% OFF</strong><span>em pagamentos<br/>via PIX</span></div><div className="hero-actions"><a className="btn-primary" href="#catalogo">Explorar perfumes <ArrowRight size={17}/></a></div></div><div className="hero-products" aria-hidden="true">{newestRecords.slice(0,3).map((product,index)=><div key={product.id} className={`hero-product hero-product-${index+1}`} style={{backgroundImage:`url(${JSON.stringify(product.images[0]?.url||product.imageUrl||"/perfume-placeholder.svg")}),url('/logo-sa-dark.png'),linear-gradient(145deg,#28241e,#0d0c0a)`,backgroundSize:"cover,72% auto,cover"}}/>)}</div></section>
